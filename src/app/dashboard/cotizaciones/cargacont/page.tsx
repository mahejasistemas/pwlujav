...
import { PDFCotizacion } from "../cargag/pdfcotizacion";
...
export default function CargaContenerizadaPage() {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem("cotizacion_cargacont_draft");
    if (savedData) {
      try {
        ...
        if (parsed.selectedCargoExtra) setSelectedCargoExtra(parsed.selectedCargoExtra);
        if (parsed.precioCargoExtra) setPrecioCargoExtra(parsed.precioCargoExtra);
      } catch {
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("cotizacion_cargacont_draft", JSON.stringify(dataToSave));
  }, [
    empresa,
    nombreCliente,
    diaExpedicion,
  ...
  const clearDraft = () => {
    localStorage.removeItem("cotizacion_cargacont_draft");
    setEmpresa("");
    setNombreCliente("");
  ...
  const allowedLoadTypes = useMemo(() => {
    ...
    if (name.includes("rabon")) return ["rabon"];
    if (name.includes("53")) return ["sencillo", "sencillo_sobrepeso"];
    if (name.includes("20") || name.includes("40")) return ["sencillo", "sencillo_sobrepeso", "full", "full_sobrepeso"];
    ...
    const loadEquipments = async () => {
      ...
      try {
        ...
        if (!equipmentsError && equipmentsData) {
          const mappedServices: GeneralCargoEquipment[] = equipmentsData.map((item: any, index: number) => ({
          ...
          const filtered = mappedServices.filter((s) => {
            const n = s.name.toLowerCase();
            return n.includes("contenedor") || n.includes("porta") || n.includes("caja seca");
          });

          setAvailableServices(filtered.sort((a, b) => a.name.localeCompare(b.name)));
  ...
  useEffect(() => {
    const fetchCompaniesAndClients = async () => {
      ...
      try {
        ...
        if (tolvasData) setTolvas(tolvasData);
        ...
        if (cargosExtrasData) setCargosExtras(cargosExtrasData);
      } catch {
      }
    };
    ...
    const fetchNextFolio = async () => {
      ...
      try {
        ...
        if (data && data.length > 0 && data[0].folio) {
          ...
          if (!isNaN(lastSeqNum)) nextSequence = lastSeqNum + 1;
        }

        const sequenceStr = nextSequence.toString().padStart(3, "0");
        setNumeroCotizacion(`${prefix}${year}${sequenceStr}`);
      } catch {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!supabase) {
        setQuoteError("Supabase no está configurado para consultar carga contenerizada.");
        return;
      }
      ...
      const tarifa = tariffData[0] as any;
      ...
      if (equipmentsToUse.length === 0) {
        const { data: equipmentsData, error: equipmentsError } = await supabase
          ...
          .select("*");
        if (equipmentsError) {
          setQuoteError("No se pudieron obtener los equipos de carga contenerizada.");
          return;
        }
      ...
      if (!candidates.length && !selectedEquipment) {
        setQuoteError("No se encontraron equipos que soporten estas medidas.");
        return;
      }
      ...
      const itemsToProcess = selectedEquipment ? [selectedEquipment] : sorted.slice(0, 2);

      for (const eq of itemsToProcess) {
        const tariffTypeToUse: string = loadType ? loadType : eq.tariff_type;
        let basePrice = 0;

        if (tariffTypeToUse === "rabon") basePrice = tarifa.rabon;
        else if (tariffTypeToUse === "sencillo") basePrice = tarifa.sencillo;
        else if (tariffTypeToUse === "sencillo_sobrepeso") basePrice = tarifa.sencillo_sobrepeso;
        else if (tariffTypeToUse === "full") basePrice = tarifa.full;
        else if (tariffTypeToUse === "full_sobrepeso") basePrice = tarifa.full_sobrepeso;
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={async () => {
              ...
              const quoteToSave = {
                ...
                monto_total: (ticketData.basePrice || 0) + (ticketData.precioTolva ? parseFloat(ticketData.precioTolva) : 0) + (ticketData.precioCargoExtra ? parseFloat(ticketData.precioCargoExtra) : 0),
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-gray-900">
            Cotización - Carga contenerizada
          </h2>
          <p className="text-sm text-gray-600 mt-1">
            Completa los datos para generar una cotización de carga contenerizada.
          </p>
        </div>
        <div className="flex gap-2">
          {ticketData && (
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid gap-4 grid-cols-2 lg:grid-cols-3 pt-2 border-t border-gray-100">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={divisa}
                      onValueChange={(value) => setDivisa(value as any)}
                    >
                      <SelectTrigger className="w-full">
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Resultados de la cotización</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                {!quoteLoading && !quoteError && quoteOptions.length === 0 && (
                  <p className="text-sm text-gray-500">
                    Completa el formulario y presiona &quot;Mostrar en mapa&quot; para ver opciones de equipos y tarifas.
                  </p>
                )}
